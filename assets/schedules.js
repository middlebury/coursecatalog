...
$("document").ready(function () {
    ...
    $("a.print_button").click(function () {
        var printView = window.open(
            this.href,
            this.target,
            "menubar=1,resizable=1,height=600,width=800,scrollbars=1"
        );
        printView.focus();
        return false;
    });
    ...
    $("#calendar").each(function () {
        renderSchedule(
            $(this),
            $(this).children("input[name=jsonUrl]").val(),
            45,
            11
        );
    });
...
function renderSchedule(calendarElement, jsonUrl, timeslotHeight, textSize) {
    if (!timeslotHeight) timeslotHeight = 20;
    if (!textSize) textSize = 13;

    var year = new Date().getFullYear();
    var month = new Date().getMonth();
    var day = new Date().getDate();

    var startHour = new Number(
        calendarElement.find("input:hidden[name=start_hour]").val()
    );
    var endHour = new Number(
        calendarElement.find("input:hidden[name=end_hour]").val()
    );
    ...
    if (
        calendarElement.find("input:hidden[name=show_sunday]").val() == "no" &&
        calendarElement.find("input:hidden[name=show_saturday]").val() == "no"
    ) {
        showWeekend = false;
    }
    ...
    const formatTime = function (date) {
        ...
        return (
            hours + ":" + minutes.toString().padStart(2, "0") + " " + meridian
        );
    };

    $.getJSON(jsonUrl, function (data) {
        console.log(data);
        const calendar = new Calendar(calendarElement.get(0), {
            defaultView: "week",
            ...
            template: {
                time(event) {
                    return `<div class="calendar_event_time">${formatTime(
                        event.start
                    )} - ${formatTime(
                        event.end
                    )}</div><div class="calendar_event_title">${
                        event.title
                    }</div><div class="calendar_event_location">${
                        event.location
                    } - CRN: ${event.raw.crn}</div>`;
                },
            },
        });
...
function getSectionSets(dialog, scheduleId) {
    ...
    $.getJSON(
        lookupUrl,
        { course: courseId, scheduleId: scheduleId },
        function (data, textStatus) {
            populateSectionSetSelect(dialog, scheduleId, data);
            populateSectionTypes(
                dialog,
                scheduleId,
                dialog.find("select.section_set").val(),
                data
            );
        }
    );
...
function populateSectionTypes(dialog, scheduleId, linkSetId, data) {
    ...
    for (var linkTypeId in data[linkSetId]["types"]) {
        ...
        for (var j = 0; j < sectionType.length; j++) {
            ...
            var radio = $(
                '<input type="radio" name="section_group_' +
                    i +
                    '" value="' +
                    section.id +
                    '"/>'
            ...
            item.append(
                ' <div class="section_instructor section_info"><span class="section_type">' +
                    section.type +
                    '</span> - <span class="section_instructor">' +
                    section.instructor +
                    "</span></div>"
            );
            item.append("");
            if (section.conflicts) {
                ...
                var conflictNames =
                    "<br/> &nbsp; &nbsp; " + section.conflictString;
            } else {
                var conflictsClass = "";
                var conflictNames = "";
            }
            item.append(
                '<div class="section_time section_info' +
                    conflictsClass +
                    '">' +
                    section.schedule +
                    conflictNames +
            ...
            item.append(
                '<div class="section_location section_info">' +
                    section.location +
                    "</div>"
            );

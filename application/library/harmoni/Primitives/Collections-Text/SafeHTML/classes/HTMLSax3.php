...
class XML_HTMLSax3_StateParser
{
    ...
    public function parse($data)
    {
        ...
        if (1 == $this->parser_options['XML_OPTION_STRIP_ESCAPES']) {
    ...
    public function _parse($state = XML_HTMLSAX3_STATE_START)
    {
        ...
        } while (XML_HTMLSAX3_STATE_STOP != $state
                    && $this->position < $this->length);
    }
}
...
class XML_HTMLSax3_StateParser_Lt430 extends XML_HTMLSax3_StateParser
{
    ...
    public function scanUntilCharacters($string)
    {
        ...
        while ($this->position < $this->length && !str_contains($string, $this->rawtext[$this->position])) {
            ++$this->position;
        }
...
class XML_HTMLSax3
{
    ...
    {
        if (version_compare(\PHP_VERSION, '4.3', 'ge')) {

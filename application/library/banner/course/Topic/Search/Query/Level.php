 ...
 * @license http://www.gnu.org/copyleft/gpl.html GNU General Public License (GPL)
 */
class banner_course_Topic_Search_Query_Level extends banner_course_AbstractQuery implements osid_course_TopicQuery
{
    /**
     * Constructor.
     *
    ...
    public function matchDisplayName($displayName, osid_type_Type $stringMatchType, $match)
    {
        ...
        if ($stringMatchType->isEqual($this->wildcardStringMatchType)) {
            ...
            $this->addClause('displayName', 'STVLEVL_DESC LIKE(?)', [$displayName], $match);
        } else {

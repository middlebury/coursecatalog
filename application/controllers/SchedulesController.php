...
class SchedulesController extends AbstractCatalogController
{
    /**
    ...
    public function preDispatch()
    {
        if (!$this->_helper->auth->getHelper()->isAuthenticated()) {
            $this->_redirect(
                $this->view->url(['controller' => 'auth', 'action' => 'login', 'return' => $this->view->url()]),
                ['exit' => true, 'prependBase' => false]);
        }
    }
    ...
    protected function getSavedCatalogId()
    {
        if (!isset($this->savedCatalogId)) {
            ...
            $stmt->execute([$this->_helper->auth()->getUserId()]);
            $row = $stmt->fetch();
            $stmt->closeCursor();
            if ($row) {
    ...
    protected function setSavedCatalogId(osid_id_Id $catalogId)
    {
        ...
        try {
            $insert->execute([$this->_helper->auth()->getUserId(), $catalogId->getAuthority(), $catalogId->getIdentifierNamespace(), $catalogId->getIdentifier()]);
            ...
            if (23000 == $e->getCode()) {
                ...
                $update->execute([$catalogId->getAuthority(), $catalogId->getIdentifierNamespace(), $catalogId->getIdentifier(), $this->_helper->auth()->getUserId()]);
            } else {
                throw $e;
            }
        }
    ...
    protected function returnToIndex()
    {
        ...
        $url = $this->view->url(['action' => 'index', 'controller' => 'schedules', 'catalog' => $catalogIdString, 'term' => $termIdString]);
        $this->_redirect($url, ['exit' => true, 'prependBase' => false]);
    }
    ...
    protected function getFromEmail()
    {
        ...
        } elseif (isset($config->schedules->email->send_mail_as) && $config->schedules->email->send_mail_as) {
